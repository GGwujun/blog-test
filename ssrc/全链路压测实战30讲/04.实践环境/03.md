...
![图片](./httpsstatic001geekbangorgresourceimage7182713cb37c39316091a6b6e987cb83c382.jpg)
...
![](./httpsstatic001geekbangorgresourceimagee53ce5500bd1ed32efe787f09d7c51d05d3c.jpg)

我们设计的上传文件的大致流程你可以看看下面这张图：

![图片](./httpsstatic001geekbangorgresourceimage4a464a304423ee40dbc8110646679e4c9f46.jpg)
...
启动成功后，访问MinIO的IP地址，因为我的 MinIO Server 安装在本机，所以是 <http://localhost:9001>，输入默认的账号密码是 minioadmin/minioadmin。

![图片](./httpsstatic001geekbangorgresourceimage6a856a99f51224acf674c7464463efc70385.png)

登录后，进入控制台看板页。

![图片](./httpsstatic001geekbangorgresourceimage117411feff640cc83593f32de5f0bf3c8074.png)
...
![图片](./httpsstatic001geekbangorgresourceimage979197d28bf2379590a77357f5513d681d91.png)

上传完成后，我们打开 MinIO 的管理界面，可以看到上传后的文件。

![图片](./httpsstatic001geekbangorgresourceimage4be74b4eca50dd7546ee1c2f9653f16419e7.png)

我们还可以调用删除接口来删除其中某文件，需要注意的是，objectName 参数值是存储桶（Buckets）中的文件相对路径，删除文件接口地址：<http://localhost:8080/minio/delete>。

![图片](./httpsstatic001geekbangorgresourceimage573d574acaf12f603b89a23e88a8a542593d.png)

最后，我们还可以调用下载接口来下载文件，下载文件接口地址：<http://localhost:8080/minio/download>。

![图片](./httpsstatic001geekbangorgresourceimage32yy32c6a4a957d88e00254f4a1c71f3dcyy.png)
...
![图片](./httpsstatic001geekbangorgresourceimagefaf2fab6fb7d04705d0c47cfb639e34769f2.png)
...
![图片](./httpsstatic001geekbangorgresourceimagedc34dc34079b07eece4b32370b950a687f34.png)

紧接着，HttpClient 会启动多个发送请求协程：

![图片](./httpsstatic001geekbangorgresourceimagee838e81f619ff63648f6b4aa34f1fee52038.png)

HttpClient 执行请求发送：

![图片](./httpsstatic001geekbangorgresourceimage81c3817fb12cae9c908672dca0a340c754c3.png)

我们可以看看HttpClient 发送请求的细节，下面这张图中，我圈出的内容是各种配置的生效点：

![图片](./httpsstatic001geekbangorgresourceimage24e62453ee30abf8f331d086d156c84afde6.png)

为了对[output\_http.go](https://github.com/buger/goreplay/blob/master/output_http.go) 有一个更清晰的理解，你可以看看下面这张核心代码逻辑调用图：

![图片](./httpsstatic001geekbangorgresourceimagec6c5c66dcecb6e3148274b13b1810d2d5cc5.png)
...
![图片](./httpsstatic001geekbangorgresourceimagefbcbfb287c1e977bb01f2c111897bf36f4cb.png)
...
![图片](./httpsstatic001geekbangorgresourceimageccb4cc4c813da3f554a6cd84290a94ed19b4.png)
...
第六步，在启动 GoReplay 进行流量回放时，查看监听端口，可以看到我们注册的端口 28081，访问接口：<http://localhost:28081/metrics>。
